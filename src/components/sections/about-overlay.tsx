...
export function AboutOverlay() {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex bg-[#090b0d]/90 backdrop-blur-[7px]">
      <div className="relative hidden flex-1 overflow-hidden lg:block">
        <Image src="/IMG_20230423_200749.jpg.jpeg" alt="Webloop Studio work" fill className="object-cover opacity-20 grayscale blur-[5px]" priority />
        <div className="absolute inset-0 bg-[#080b0d]/75" />
        <p className="absolute bottom-10 left-10 max-w-sm font-sora text-5xl font-bold uppercase leading-[0.9] tracking-[-0.07em] text-white/20 xl:text-7xl">
      ...
      <section className="relative h-full w-full overflow-y-auto bg-[#f0f0ec] text-[#111315] shadow-[-18px_0_60px_rgba(0,0,0,0.3)] lg:w-[min(57vw,920px)] lg:min-w-[620px]">
        <Link href="/" aria-label="Close About page" className="absolute right-5 top-5 z-10 inline-flex h-8 items-center gap-2 bg-[#101214] px-3 font-mono text-[11px] font-bold uppercase text-white transition-transform hover:-translate-y-0.5">
          Close <X className="h-3.5 w-3.5" />
        </Link>

        <div className="px-7 pb-16 pt-8 sm:px-12 sm:pt-9 lg:px-14">
          <div className="mb-14 flex items-center gap-3 pr-24 font-mono text-[10px] uppercase tracking-[0.08em] text-[#62635f]">
            <span className="h-3 w-3 rounded-full bg-[#999b95]" />
            <span className="font-sans text-sm font-bold tracking-normal text-[#111315]">About the studio</span>
          </div>
          ...
          <div className="relative mt-5 aspect-[0.82] overflow-hidden bg-[#b9b0a3] sm:aspect-[1.05]">
            <Image src="/IMG_20230423_200749.jpg.jpeg" alt="A Webloop Studio creative session" fill className="object-cover" />
            <span className="absolute left-7 top-7 font-sora text-5xl font-semibold tracking-[-0.08em] text-white sm:text-7xl">listen</span>
            <span className="absolute right-7 top-[38%] font-sora text-5xl font-semibold tracking-[-0.08em] text-white sm:text-7xl">create</span>
            <span className="absolute bottom-[24%] left-7 font-sora text-5xl font-semibold tracking-[-0.08em] text-white sm:text-7xl">obsess</span>
          </div>
